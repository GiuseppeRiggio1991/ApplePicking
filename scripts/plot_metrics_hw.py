...
# eucl_dir = '/home/fred/hydra_ws/src/sawyer_planner/results/hw/euclidean'
fred_dir = '/home/fred/hydra_ws/src/sawyer_planner/results/hw'
...
planner_times_avg = [numpy.nanmean(row) for row in planner_times_arr[0]]
...
# plt.figure()
# print(planner_times_arr)
sequencing_times_avg = [numpy.nanmean(row) for row in sequencing_times_arr[0]]
# for mat in sequencing_times_arr:
#     sequencing_times_avg.append(numpy.nanmean([numpy.nanmean(row) for row in mat]))
#     print [numpy.nanmean(row) for row in mat]
# performance = [10,8,6,4,2,1]
...
# plt.figure()
# # print(planner_times_arr)
planner_executions_avg = [numpy.nanmean(row) for row in planner_executions_arr[0]]
...
approach_times_avg = [numpy.nanmean(row) for row in approach_times_arr[0]]
...
# plt.show()
print planner_times_avg
success_rates = [100, 75, 100, 100, 75, 87.5, 100, 75, 75, 100]
it = 1
for plan_time, plan_duration, approach, success in zip(planner_times_avg, planner_executions_avg, approach_times_avg, success_rates):
	print("\\hline")
	print(str(it) + ' & ' + '{0:.2f}'.format(plan_time) + ' & ' + '{0:.2f}'.format(plan_duration) + ' & ' + '{0:.2f}'.format(approach) + 
			' &  ' + '{0:.2f}'.format(success) + ' \\\\' )
	it += 1

print("\\hline")
print('Global & ' + '{0:.2f}'.format(numpy.mean(planner_times_avg)) + ' & ' + '{0:.2f}'.format(numpy.mean(planner_executions_avg)) + ' & ' + 
		'{0:.2f}'.format(numpy.mean(approach_times_avg)) + ' & ' + '{0:.2f}'.format(numpy.mean(success_rates)) + ' \\\\')